...
data class Key(val value: ByteArray) {
    ...
    companion object {
        fun fromBase64(base64: String): Key = Key(Base64.getDecoder().decode(base64))
    }
}

object Aead {
    // NOW Writes directly to output buffer
    fun chacha20Poly1305Encrypt(
        key: Key, 
        counter: Long, 
        plaintext: ByteArray, plainOffset: Int, plainLength: Int,
        associatedData: ByteArray,
        output: ByteArray, outOffset: Int
    ): Int {
        val nonce = ByteArray(12).apply { 
            ByteBuffer.wrap(this, 4, 8).order(ByteOrder.LITTLE_ENDIAN).putLong(counter) 
        }
        val cipher = ChaCha20Poly1305()
        cipher.init(true, AEADParameters(KeyParameter(key.value), AUTH_TAG_LENGTH * 8, nonce, associatedData))
        
        var len = cipher.processBytes(plaintext, plainOffset, plainLength, output, outOffset)
        len += cipher.doFinal(output, outOffset + len)
        return len
    }

    // AND Decrypts directly into output buffer
    fun chacha20Poly1305Decrypt(
        key: Key, 
        counter: Long, 
        ciphertext: ByteArray, cipherOffset: Int, cipherLength: Int,
        associatedData: ByteArray,
        output: ByteArray, outOffset: Int
    ): Int {
        return try {
            ...
            var len = cipher.processBytes(ciphertext, cipherOffset, cipherLength, output, outOffset)
            len += cipher.doFinal(output, outOffset + len)
            len
        } catch (e: Exception) { -1 }
    }
    ...
    fun chacha20Poly1305Encrypt(key: Key, counter: Long, plaintext: ByteArray, associatedData: ByteArray): ByteArray {
        val out = ByteArray(plaintext.size + AUTH_TAG_LENGTH)
        chacha20Poly1305Encrypt(key, counter, plaintext, 0, plaintext.size, associatedData, out, 0)
        return out
    }

    fun chacha20Poly1305Decrypt(key: Key, counter: Long, ciphertext: ByteArray, associatedData: ByteArray): ByteArray? {
        val out = ByteArray(ciphertext.size - AUTH_TAG_LENGTH)
        val len = chacha20Poly1305Decrypt(key, counter, ciphertext, 0, ciphertext.size, associatedData, out, 0)
        return if (len >= 0) out else null
    }
...
private fun hmac(key: ByteArray, vararg data: ByteArray): ByteArray {
    val hmac = HMac(Blake2sDigest(256))
...
fun kdf2(ck: ByteArray, input: ByteArray): Pair<ByteArray, Key> {
    ...
    val key2 = hmac(temp, com.google.common.primitives.Bytes.concat(key1, byteArrayOf(0x02)))
...
fun kdf3(ck: ByteArray, input: ByteArray): Triple<ByteArray, Key, Key> {
    ...
    val key1 = hmac(temp, byteArrayOf(0x01))
    val key2 = hmac(temp, com.google.common.primitives.Bytes.concat(key1, byteArrayOf(0x02)))
    val key3 = hmac(temp, com.google.common.primitives.Bytes.concat(key2, byteArrayOf(0x03)))
